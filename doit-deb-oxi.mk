...
VAGDIR	= $(HOME)/git/vagrant/debian
VAGINST = build
VAGPROV = virtualbox
VAGID = $(VAGDIR)/machines/$(VAGINST)/$(VAGPROV)/id
VAGSSHCFG = $(VAGDIR)/$(VAGINST).sshcfg
...
myperl: $(VAGSSHCFG)
	ssh -F $< build \
		"cd ~/git/myperl && make fetch-perl debian debian-install"
